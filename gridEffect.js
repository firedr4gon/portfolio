let theme = "rgba(255,255,255,";
let themeContrastColor = "white";
...
class App {
  ...
  getTextPixels() {
    ...
    tools.drawPath(ctx, () => {
      ctx.fillStyle = themeContrastColor;
  ...
  startGeneratingNumbers() {
    ...
    function generateItem() {
      ...
      const item = {
        p: 0,
        color: `${theme}${tools.random(0.01, 0.3)})`,
  ...
  drawNumbersAnimation() {
    ...
    state.animNumbers.forEach((item, i) => {
      ...
      tools.drawPath(ctx, () => {
        if (cfg.shadowBlur) {
          ctx.shadowColor = themeContrastColor;
          ctx.shadowBlur = 10;
        }
        ctx.globalAlpha = state.fadeInProgress;
  ...
  drawAnimLines() {
    ...
    state.animLines.forEach((line, i) => {
      ...
      const color = `${theme}${0.1 + line.color * (p <= 0.5 ? p1 : p2)})`;
  ...
  startGeneratingGlitches() {
    ...
    function generateItem() {
      ...
      const item = {
        p: 0,
        color: `${theme}${tools.random(0.01, 1)})`,
        blinks: Array(tools.random(0, 3, true))
          .fill(null)
          .map((blink) => {
            return {
  ...
  drawMouseMoveInteraction(props) {
    ...
    if (vector <= mouseRange) {
      ...
      const color = `${theme}${0.3 * percent})`;
      const color2 = `${theme}${0.7 * p2 * percent})`;
      ...
      tools.drawPath(ctx, () => {
        ...
        ctx.strokeStyle = `${theme}${0.75 * percent})`;
  ...
  drawYLines(props) {
    ...
    if (i) {
      tools.drawPath(ctx, () => {
        ctx.fillStyle = `${theme}${0.05 + (1 - p) * 0.35})`;
  ...
  drawYMarkup(props) {
    ...
    ctx.fillStyle = `${theme}${0.1 + (1 - f) * 0.75})`;
    const textCoords = [
...
darkmodeButton.addEventListener("click", () => {
  ...
  if (theme == "rgba(255,255,255,") {
    theme = "rgba(0,0,0,";
    themeContrastColor = "black";
  } else {
    theme = "rgba(255,255,255,";
  ...
  window.app.bindNodes();
  window.app.getDimensions();
  mouse.x = doc.width / 2;
  mouse.y = doc.height / 2;
  window.app.start();
});
